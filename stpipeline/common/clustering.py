...
def _get_connected_components_adjacency(adj_list: Dict[str, List[str]], counts: Counter) -> List[str]:
    """
...
def _get_adj_list_adjacency(umis: List[str], allowed_mismatches: int) -> Dict[str, List[str]]:
	...
	return {
		umi: [
			umi2
			for umi2 in umis
			if hamming_distance(umi.encode("UTF-8"), umi2.encode("UTF-8"))
			<= allowed_mismatches
		]
		for umi in umis
	}

def _get_best_adjacency(
	cluster: List[str], adj_list: Dict[str, List[str]], counts: Counter
) -> List[str]:
	"""
...
def _reduce_clusters_adjacency(
	adj_list: Dict[str, List[str]], clusters: List[List[str]], counts: Counter
) -> List[str]:
	"""
	...
	for cluster in clusters:
		parent_umis = _get_best_adjacency(cluster, adj_list, counts)
		unique_umis += parent_umis
	return unique_umis

def _get_adj_list_directional_adjacency(umis: List[str], counts: Counter, allowed_mismatches: int) -> Dict[str, List[str]]:
	"""
...
def _reduce_clusters_directional_adjacency(clusters: List[Set[str]]) -> List[str]:
	"""
...
def dedup_hierarchical(
    ...
    if len(molecular_barcodes) == 2:
        return molecular_barcodes if hamming_distance(
            molecular_barcodes[0].encode("UTF-8"),
            molecular_barcodes[1].encode("UTF-8")
        ) <= allowed_mismatches else [random.choice(molecular_barcodes)]

    def d(coord):
        i, j = coord
        return hamming_distance(
            molecular_barcodes[i].encode("UTF-8"),
...
def dedup_adj(
    ...
    adj_list = _get_adj_list_adjacency(c.keys(), allowed_mismatches)
...
def dedup_dir_adj(
    ...
    if len(molecular_barcodes) == 0:
        raise RuntimeError("The input UMIs cannot be empty")
    c = Counter(molecular_barcodes)
    adj_list = _get_adj_list_directional_adjacency(c.keys(), c, allowed_mismatches)
    clusters = _get_connected_components_adjacency(adj_list, c)
